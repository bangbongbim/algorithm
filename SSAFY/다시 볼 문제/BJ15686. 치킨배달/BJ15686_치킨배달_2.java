...
public class BJ15686_치킨배달_2 {
	...
	static void comb(int srcIdx, int tgtIdx) {
		if (tgtIdx == M) {
			// complete code
			// 현재 치킨집 조합을 기준으로 모든 집에서 최단거리 치킨집을 계산해서 모두 더함 => 그 합의 최소값 선택

			int sum = 0; // 치킨 거리의 합
			int size = house.size();

			for (int i = 0; i < size; i++) { // 모든 집 각각에 대해서 고려
				int dist = Integer.MAX_VALUE; // i 번째 집의 치킨 거리
		...
		tgt.remove(src.get(srcIdx));// 배열이 아닌 List이므로 명시적으로 삭제하는 코드
		comb(srcIdx + 1, tgtIdx + 1);
