...
//     System.setIn(new FileInputStream("SWEA6808.txt"));
	...
	static void bfs() {
		while (!queue.isEmpty()) {
			// visit는 필요가 없다! 어차피 다음으로 방문하는 것은 +1? 이기 때문
			Node node = queue.poll();

			// 문제의 답 체크
			if (node.cnt > COUNT) {
				COUNT = node.cnt; // 최대 방문 수 갱신
				NO = node.no; // 출발할 때 방 번호
			} else if (node.cnt == COUNT) {
				// 두 개가 같으면 방번호 더 작은 값으로
			...
			for (int d = 0; d < 4; d++) {
				...
				break; // 조건에 맞는 다음 방 번호는 한 개이므로 더 이상 delta를 따져 볼 필요가 없다.
			}
